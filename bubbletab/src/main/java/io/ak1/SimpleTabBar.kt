...
class SimpleTabBar : LinearLayout {
    ...
    fun setSelected(position: Int, callListener: Boolean = true) {
        val it = (this@SimpleTabBar.getChildAt(position) as SimpleTab)
    ...
    fun setSelectedWithId(@IdRes id: Int, callListener: Boolean = true) {
        val it = this@SimpleTabBar.findViewById<SimpleTab>(id) ?: return
        val b = it.id
        if (oldBubble != null && oldBubble!!.id != b) {
            it.isSelected = !it.isSelected
            oldBubble!!.isSelected = false
    ...
    private var oldBubble: SimpleTab? = null
        ...
        menu.forEach { it ->
            ...
            addView(SimpleTab(context, it).apply {
                ...
                setOnClickListener {
                    val b = it.id
                    if (oldBubble != null && oldBubble!!.id != b) {
                        (it as SimpleTab).isSelected = !it.isSelected
                        oldBubble!!.isSelected = false
                    }
                    oldBubble = it as SimpleTab
